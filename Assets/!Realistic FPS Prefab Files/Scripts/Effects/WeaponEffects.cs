//WeaponEffects.cs by Azuline StudiosВ© All Rights Reserved
...
public class WeaponEffects : MonoBehaviour {
	...
	public GameObject EffectsParentObj { get; set; }
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Draw Impact Effects
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void ImpactEffects ( RaycastHit hit  ){
		if(_fpsPlayer.CurrentWeaponBehavior.meleeSwingDelay == 0){//not a melee weapon
	    ...
	    foreach (Transform child in impactObj.transform){//emit all particles in the particle effect game object group stored in impactObj var
			Transform childParticleTransform = child;//cache child particle transform ref for efficiency
		...
		if(_fpsPlayer.CurrentWeaponBehavior.projectileCount > 1){
			hitVolumeAmt = 0.35f;	
		}else if(!_fpsPlayer.CurrentWeaponBehavior.semiAuto){
			hitVolumeAmt = 0.8f;	
		}else{
			hitVolumeAmt = 1.0f;
		}
	...
	public void ImpactEffects(RaycastHit hit, WeaponBehavior weaponBehavior)
	{
		if (weaponBehavior.meleeSwingDelay == 0f)
		{
			switch (hit.collider.gameObject.tag)
			{
			case "Dirt":
				...
				{
					hitSound = defaultImpactSounds[UnityEngine.Random.Range(0, defaultImpactSounds.Length)];
				}
				break;
			case "Metal":
				impactObj = metalImpact;
				hitSound = metalImpactSounds[UnityEngine.Random.Range(0, metalImpactSounds.Length)];
				break;
			case "Wood":
				impactObj = woodImpact;
				if ((bool)woodImpactSounds[0])
				{
					hitSound = woodImpactSounds[UnityEngine.Random.Range(0, woodImpactSounds.Length)];
				}
				break;
			case "Water":
				impactObj = waterImpact;
				...
				{
					hitSound = waterImpactSounds[UnityEngine.Random.Range(0, waterImpactSounds.Length)];
				}
				break;
			case "Glass":
				impactObj = glassImpact;
				if ((bool)waterImpactSounds[0])
				{
					hitSound = glassImpactSounds[UnityEngine.Random.Range(0, glassImpactSounds.Length)];
				}
				break;
			case "Flesh":
				impactObj = fleshImpact;
				...
				{
					hitSound = fleshImpactSounds[UnityEngine.Random.Range(0, fleshImpactSounds.Length)];
				}
				break;
			case "Zombie":
				impactObj = fleshImpact;
			...
			case "Stone":
				...
				{
					hitSound = stoneImpactSounds[UnityEngine.Random.Range(0, stoneImpactSounds.Length)];
				}
				break;
			default:
				impactObj = dirtImpact;
		...
		else
		{
			...
			case "Dirt":
				impactObj = dirtImpactMelee;
				hitSound = defaultImpactSoundsMelee[UnityEngine.Random.Range(0, defaultImpactSoundsMelee.Length)];
				break;
			case "Metal":
				impactObj = metalImpactMelee;
				hitSound = metalImpactSoundsMelee[UnityEngine.Random.Range(0, metalImpactSoundsMelee.Length)];
				break;
			case "Wood":
				impactObj = woodImpactMelee;
				hitSound = woodImpactSoundsMelee[UnityEngine.Random.Range(0, woodImpactSoundsMelee.Length)];
				break;
			case "Water":
				impactObj = waterImpact;
				hitSound = waterImpactSounds[UnityEngine.Random.Range(0, waterImpactSounds.Length)];
			...
			case "Flesh":
				impactObj = fleshImpact;
				hitSound = fleshImpactSoundsMelee[UnityEngine.Random.Range(0, fleshImpactSoundsMelee.Length)];
				break;
			case "Zombie":
				impactObj = fleshImpact;
				hitSound = fleshImpactSoundsMelee[UnityEngine.Random.Range(0, fleshImpactSoundsMelee.Length)];
			...
			case "Stone":
				impactObj = stoneImpactMelee;
				hitSound = stoneImpactSoundsMelee[UnityEngine.Random.Range(0, stoneImpactSoundsMelee.Length)];
				break;
			default:
				impactObj = dirtImpactMelee;
				hitSound = defaultImpactSoundsMelee[UnityEngine.Random.Range(0, defaultImpactSoundsMelee.Length)];
		...
		//modify the weapon impact sounds based on the weapon type, so the multiple shotgun impacts and automatic weapons aren't so loud
		if(weaponBehavior.projectileCount > 1){
			hitVolumeAmt = 0.35f;	
		}else if(!weaponBehavior.semiAuto){
			hitVolumeAmt = 0.8f;	
		}else{
			hitVolumeAmt = 1.0f;
		}
	...
	public void BulletMarks ( RaycastHit hit, WeaponBehavior behavior  ){
		if(behavior.meleeSwingDelay == 0){//not a melee weapon
			//find the tag name of the hit game object and select an impact mark for the surface type
			switch(hit.collider.gameObject.tag){
			case "Dirt":
				markObj = dirtMarks[Random.Range(0, dirtMarks.Length)];
