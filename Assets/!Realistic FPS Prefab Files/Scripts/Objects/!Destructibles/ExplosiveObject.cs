//ExplosiveObject.cs by Azuline StudiosВ© All Rights Reserved
    ...
    IEnumerator Detonate() {
		...
		if(explosionEffect){
		    foreach (Transform child in explosionEffect){//emit all particles in the particle effect game object group stored in explosionEffect var
				...
				child.rotation = Quaternion.FromToRotation(Vector3.up, Vector3.up);//rotate effects so they are vertical
