...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	void Update(){
		...
		if(Input.GetKeyDown (FPSPlayerComponent.crouch) && CanUseInput && !swimming && !climbing){
			if(!crouchState){
				if(!crouched){
					crouched = true;
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(FPSPlayerComponent.hitPoints > 1.0f){
				...
				if(!Physics.CapsuleCast (sweepPos, sweepHeight, capsule.radius * 0.5f, myTransform.forward, out hit2, 0.4f + (playerHeightMod * 0.2f), clipMask.value) || climbing) {
					if(!FPSPlayerComponent.useAxisInput){
						...
						if(Input.GetKey (FPSPlayerComponent.moveForward) && CanUseInput){inputY = 1;}
				...
				if(!FPSPlayerComponent.useAxisInput){//only check keyboard buttons if not using Unity's input axes
					...
					if (Input.GetKey (FPSPlayerComponent.moveBack) && CanUseInput){inputY = -1;}
					if (!Input.GetKey (FPSPlayerComponent.moveBack) && !Input.GetKey (FPSPlayerComponent.moveForward) || !CanUseInput){inputY = 0;}
					if (Input.GetKey (FPSPlayerComponent.moveBack) && Input.GetKey (FPSPlayerComponent.moveForward) || !CanUseInput){inputY = 0;}
					if (Input.GetKey (FPSPlayerComponent.strafeLeft) && CanUseInput){inputX = -1;}
					if (Input.GetKey (FPSPlayerComponent.strafeRight) && CanUseInput){inputX = 1;}
					if (!Input.GetKey (FPSPlayerComponent.strafeLeft) && !Input.GetKey (FPSPlayerComponent.strafeRight) || !CanUseInput){inputX = 0;}
			...
			if (grounded) {
				...
				//Sprinting
				/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				bool key = Input.GetKey(FPSPlayerComponent.sprint);
				if (key && CanUseInput)
				{
					if (Mathf.Abs(inputY) > 0.1f)
					{
						if ((!FPSPlayerComponent.useAxisInput && Input.GetKey (FPSPlayerComponent.moveForward) && (bool)CanUseInput) || (FPSPlayerComponent.useAxisInput && Mathf.Abs(inputY) > 0.1f))
						{
				...
				if((sprintActive && Input.GetAxis("Fire1") > 0.0f)//cancel sprint if fire button is pressed
					|| (sprintActive && Input.GetKey(FPSPlayerComponent.reload) && CanUseInput)//cancel sprint if reload button is pressed
					|| (sprintActive && Input.GetKey(FPSPlayerComponent.zoom) && CanUseInput && WeaponBehaviorComponent.canZoom)//cancel sprint if zoom button is pressed
					|| (FPSPlayerComponent.zoomed && Input.GetKey (FPSPlayerComponent.fire) && CanUseInput)
			...
			if((climbing || swimming) && !jumping){//climbing var is managed by a ladder script attatched to a trigger that is placed near a ladder
				if((!FPSPlayerComponent.useAxisInput && Input.GetKey(FPSPlayerComponent.moveForward) && CanUseInput) || (FPSPlayerComponent.useAxisInput && inputY > 0)){//only climb if player is moving forward
			...
			if((grounded || climbing || swimming || ((airTime + 0.3f) > Time.time)) && FPSPlayerComponent.hitPoints > 1.0f && !FPSPlayerComponent.restarting){
				...
				if(!capsuleTooSteep || climbing || swimming || (capsuleTooSteep && !rayTooSteep)){
					...
					if(climbing){
						if((!FPSPlayerComponent.useAxisInput && Input.GetKey (FPSPlayerComponent.moveForward)) || (FPSPlayerComponent.useAxisInput && Mathf.Abs(inputY) > 0.1f)){//move player up climbable surface if pressing forward button
							velocityChange.y = verticalSpeedAmt;
						}else if(Input.GetKey(FPSPlayerComponent.jump) && CanUseInput){//move player up climbable surface if pressing jump button
							inputY = 1;//to cycle bobbing effects
							if(FPSPlayerComponent.useAxisInput){inputYSmoothed = 1.0f;}
							velocityChange.y = climbSpeed * 0.75f;
						}else if(Input.GetKey(FPSPlayerComponent.crouch) && CanUseInput){//move player down climbable surface if pressing crouch button
							inputY = -1;//to cycle bobbing effects
							if(FPSPlayerComponent.useAxisInput){inputYSmoothed = 1.0f;}
							velocityChange.y = -climbSpeed * 0.75f;
						}else{
			...
			if(!climbing){
				if(!swimming){
					...
					if(swimStartTime + 0.2f > Time.time){//make player sink under surface for a short time if they jumped in deep water 
						...
						if(Input.GetKey(FPSPlayerComponent.jump) && CanUseInput){
						...
						}else if(Input.GetKey(FPSPlayerComponent.crouch) && CanUseInput){
