...
public class PhotonMob : Photon.MonoBehaviour
{
	...
	[SerializeField] ZombieType _mobType;
	[SerializeField] MobAI _ai;
	[SerializeField] GameObject deadBody;
	[SerializeField] ZombieSoundController _soundController;
	[SerializeField] PhotonTransformView m_TransformView;
	...
	public void Hit(short damage, Vector3 attackerPos, Vector3 attackerDir, bool isPlayer, string attackerSocialId)
	{
	...
	public void ActivateMobByMaster(bool activate, string mobId, Vector3 position)
	{
		if (activate)
		{
			if (!MobIsActive)
	...
	private void ActivateMob(string mobId)
	{
		...
		{
			Mob mobInfo = WorldController.I.Info.GetMobInfo(mobId);
			if (mobInfo != null)
			{
	...
	private void SyncInfo(string id, short hp)
	{
		...
		{
			Mob mobInfo = WorldController.I.Info.GetMobInfo(id);
			if (mobInfo != null)
			{
	...
	private void AddFragToKiller(string attackerSocialId, PhotonPlayer killer)
	{
	...
	[PunRPC]
	private void HitMob(short damage, Vector3 attackerPos, Vector3 attackerDir, bool isPlayer, string attackerSocialId, PhotonMessageInfo info)
	{
		...
		ActivateMobByMaster(false, null, base.transform.position);
		...
		foreach (DropItem dropItem in _info.DropItems)
		{
			...
			if (valueInt > 0)
			{
				...
				RaycastHit hitInfo;
				if ((float)num2 < DataKeeper.DefaultMobDropChance && Physics.Raycast(base.transform.position + Vector3.up, -Vector3.up, out hitInfo, 100f, ItemsRegistry.I.ItemsCollisionsMask))
				{
