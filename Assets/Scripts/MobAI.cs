...
public class MobAI : Photon.MonoBehaviour
{
	...
	[SerializeField] bool _agentUpdateRotation;
	[SerializeField] float _rotationSpeed = 5f;
	[SerializeField] float _attackHeightOffset;
	...
	public void EnableAI()
	{
		if (!Enabled)
		{
			...
			if (UnityEngine.AI.NavMesh.SamplePosition(base.transform.position, out hit, 500f, 1))
			{
			...
			{
				Debug.Log("NavMesh.SamplePosition couldn't find point");
			}
		}
	}
		...
		if (Vector3.Dot(direction.normalized, base.transform.forward) > 0.4f)
		{
			...
			if (Physics.Raycast(vector, direction, out hitInfo, AttackDistance, AttackCullingMask))
			{
				...
				case 25:
				{
					...
					{
						component.HitPlayer((short)_damage, _radiation, null);
						base.photonView.RPC("OnAttackTarget", PhotonTargets.All);
					}
					break;
				}
	...
	private void DayTimeBalanceLogic()
	{
		...
		if (WorldController.I != null && WorldController.I.IsDay)
		{
			if (!_isDayLogic)
			{
