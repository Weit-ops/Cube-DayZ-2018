...
public class PhotonMan : Photon.MonoBehaviour, IPunObservable
{
	...
	public void ShowPlayerAttack(ObscuredVector3? targetPoint)
	{
		...
		if (nearPhotonPlayers != null)
		{
			...
			{
				base.photonView.RPC("ShowAttackAtPoint", PhotonTargets.All,list.ToArray(), (short)targetPoint.Value.x, (short)targetPoint.Value.y, (short)targetPoint.Value.z);
			}
			else
			{
				base.photonView.RPC("ShowAttack",PhotonTargets.All, list.ToArray());
			}
		}
	}
