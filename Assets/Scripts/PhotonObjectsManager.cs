...
{
	[SerializeField] float _syncInterval;
	[HideInInspector]
	...
	private void ActivateObject(short index, bool activate)
	{
		...
		if (Objects [index].Animation != null) 
		{
			...
			if (Objects [index].Animation[animation] == null) {
				animation = ((!activate) ? "Disable_0" : "Enable_0");
			}
	...
	private void HitObject(short index, short damage)
	{
		...
		foreach (DropItem dropItem in Objects[index].Info.DropItems)
		{
			...
			RaycastHit hitInfo;
			if (Physics.Raycast(origin, -Objects[index].transform.up, out hitInfo, 100f, ItemsRegistry.I.ItemsCollisionsMask))
			{
