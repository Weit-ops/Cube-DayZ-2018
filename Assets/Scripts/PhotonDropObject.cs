...
public class PhotonDropObject : Photon.MonoBehaviour
{
	...
	{
		Item itemInfo = WorldController.I.Info.GetItemInfo(ObjInfo.ObjectId);
		_model = (GameObject)Object.Instantiate(Resources.Load(WorldController.I.GetResourceLoadPath(itemInfo.Type) + itemInfo.Prefab));
	...
	public void PickUpItem()
	{
		...
		{
			PhotonPlayer[] nearPhotonPlayers = WorldController.I.Player.SectorController.GetNearPhotonPlayers(WorldController.I.Player.transform.position, 10f);
			if (nearPhotonPlayers != null)
			{
				if (nearPhotonPlayers.Length < 2)
				{
					GetItemLocal();
				}
				else
	...
	private void GetItemLocal()
	{
		...
		if (num > 0)
		{
			if (num2 > 0)
			{
				...
				GameControls.I.Player.PlayPickUpSnd();
				return;
			}
		}
		else
		...
		GameControls.I.Player.PlayPickUpFailSnd();
	}
