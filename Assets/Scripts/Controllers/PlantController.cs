...
{
	[SerializeField] float _growTimeInSec = 60f;
	[SerializeField] GameObject _plantModel;
	...
	private void Ripe()
	{
		...
		{
			Item itemInfo = WorldController.I.Info.GetItemInfo(_plantId);
			_plantModel.SetActive(false);
			RaycastHit hitInfo;
			if (itemInfo != null && Physics.Raycast(base.transform.position + Vector3.up, -Vector3.up, out hitInfo, 10f, ItemsRegistry.I.ItemsCollisionsMask))
			{
