...
public class InventorySlot
{
	...
	public ObscuredInt Count { get; private set; }
	public ObscuredInt MaxItemCount { get; private set; }
	public ObscuredByte Ammo { get; private set; }
	...
	public int AddSome(Item item, int count, byte ammo = 0)
	{
		...
		if (Item == null)
		{
			...
			{
				Weapon weaponInfo = WorldController.I.Info.GetWeaponInfo(item.Id);
	...
	public void Drop(Transform playerPos, int count, bool killedByPlayer)
	{
		...
		if (!killedByPlayer)
		{
			...
			{
				if (!((ObscuredString)worldPlayer.Key != WorldController.I.MyId) || !(Vector3.Distance(playerPos.position, worldPlayer.Value.transform.position) < DataKeeper.DistanceToDropLocalLoot))
				{
					continue;
				}
				...
				if (DataKeeper.BackendInfo.user.has_premium)
				{
					...
					if (num < DataKeeper.PhotonDropChanceOnPremium)
					{
						flag = true;
					}
				}
		...
		if (flag)
		{
			...
			if (Physics.Raycast(playerPos.position, -Vector3.up, out hitInfo, 100f, ItemsRegistry.I.ItemsCollisionsMask))
			{
...
public class InventoryController
{
	...
	public void EquipFromPack(Item item, int additionalCount)
	{
		...
		{
			int weaponIndex = ItemsRegistry.I.GetWeaponIndex(Equipment["Hand"]);
		...
		{
			text = "Hand";
			int weaponIndex2 = ItemsRegistry.I.GetWeaponIndex(item.Id);
			Weapon weaponInfo = WorldController.I.Info.GetWeaponInfo(item.Id);
		...
		if (!string.IsNullOrEmpty(_takeOffBuffer.Key))
		{
			...
			{
				Item itemInfo = WorldController.I.Info.GetItemInfo(_takeOffBuffer.Key);
				if (itemInfo != null)
				{
	...
	public void Equip(InventorySlot slot, Action<string, string> equipCallback = null)
	{
		if (slot.Item == null)
		{
			return;
		...
		{
			Weapon weaponInfo = WorldController.I.Info.GetWeaponInfo(Equipment["Hand"]);
			if (weaponInfo != null)
			{
				int weaponIndex = ItemsRegistry.I.GetWeaponIndex(Equipment["Hand"]);
				if (weaponIndex < GameControls.I.PlayerWeapons.WeaponsBehavioursList.Count)
		...
		{
			text = "Hand";
			int weaponIndex2 = ItemsRegistry.I.GetWeaponIndex(slot.Item.Id);
			Weapon weaponInfo2 = WorldController.I.Info.GetWeaponInfo(slot.Item.Id);
	...
	public void OnDie(bool killedByPlayer)
	{
		...
		{
			byte ammo = 0;
			if (item == "Hand" && WorldController.I.Info.GetWeaponInfo(Equipment["Hand"]) != null)
			{
				int weaponIndex = ItemsRegistry.I.GetWeaponIndex(Equipment["Hand"]);
				if (weaponIndex < GameControls.I.PlayerWeapons.WeaponsBehavioursList.Count)
				{
	...
	public void TakeOff(string key, bool returnToInventory = true, bool emptyHand = true, bool addToBuffer = false, byte ammo = 0)
	{
		if (Equipment.ContainsKey(key))
		{
			...
			Item itemInfo = WorldController.I.Info.GetItemInfo(text);
			...
			case ItemType.Weapon:
			{
				int weaponIndex = ItemsRegistry.I.GetWeaponIndex(itemInfo.Id);
				if (weaponIndex < GameControls.I.PlayerWeapons.WeaponsBehavioursList.Count)
				{
