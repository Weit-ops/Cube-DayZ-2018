...
public class Controller : MonoBehaviour
{
	...
	private void Start()
	{
		inputName.Text = "Гость " + UnityEngine.Random.Range (3000, 400040); 
		if (PlayerPrefs.HasKey("logout3"))
		{
			string @string = PlayerPrefs.GetString("logout3");
			Debug.Log("Logout " + @string);
			if (@string == "true")
			{
				_isLogout = true;
			}
		...
		if (CheckIfGuestExist() && !_isLogout)
		{
			GuestAuth();
	...
	private void GuestAuth()
	{
		...
		if (text.Length < 4)
		{
			SmallNickErrObj.gameObject.SetActive(true);
		} 
		else 
		{
			if (text == string.Empty || text == " ") 
			{
				text = "Гость " + UnityEngine.Random.Range (1000, 2000000);
			}
	...
	public void VkAuthClick()
	{
		VKApiClient.I.Authenticate ();
	}
	...
	public void OnAuthResponse(string data)
	{
		...
		if (jsonData ["IsVip"].ToString () == "true") 
		{
			...
			DataKeeper.BackendInfo.user.premium_vip_end_time = jsonData ["vip_endTime"].ToString ();
		} 
		if (jsonData ["IsSuperVip"].ToString () == "true") 
		{
			DataKeeper.BackendInfo.user.has_premium = true;
			DataKeeper.BackendInfo.user.premium_end_time = jsonData ["superVip_endTime"].ToString ();
		} 
