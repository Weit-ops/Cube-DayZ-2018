...
public class WeaponBehavior : MonoBehaviour {
	...
	{
		if (!_ironsights.reloading && (int)ammo > 0 && (int)bulletsLeft < (int)bulletsPerClip && shootStartTime + fireRate < Time.time && !Input.GetKey(_fpsPlayer.fire) /*&& GameControls.I.Player != null && !GameControls.I.Player.inTheCar*/)
		{
		...
		if(_isInitialized && Time.timeScale > 0 && Time.deltaTime > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed){
				...
				if(_fpsPlayer.hitPoints >= 1.0f){
					...
					&& !Input.GetKeyDown (_fpsPlayer.fire)
					...
					if (Input.GetKey (_fpsPlayer.reload) 
					&& !_ironsights.reloading 
					&& ammo > 0 
					&& bulletsLeft < bulletsPerClip
					&& shootStartTime + fireRate < Time.time
					...
					if(!_fpsWalker.hideWeapon){//dont shoot if player is climbing
						...
						|| Input.GetKey (_fpsPlayer.fire)){
							...
							//sprinting
							if((!_fpsPlayer.useAxisInput && (Input.GetKey (_fpsPlayer.moveForward) || Input.GetKey (_fpsPlayer.moveBack)))
					...
					if (Input.GetKey(_fpsPlayer.fire)){
						if((cantFireState)
						&& (canShoot)
						&& (bulletsLeft <= 0)
						&& (ammo <= 0)
					...
					//Change fire mode
					if (Input.GetKey(_fpsPlayer.fireMode)){
						if(fireModeState
						&& canShoot
						&& !_ironsights.reloading
					...
					if (barrelSmokePos){
						if(shootStartTime + 0.25f > Time.time){					
							if(barrelSmokeParticles && bulletsJustFired >= barrelSmokeShots ){
								...
								|| ((shootStartTime + fireRate < Time.time) && Input.GetKey(_fpsPlayer.fire))){
	...
	void Update (){
		if(_isInitialized && _playerWeapons.CanShoot && Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed && _fpsPlayer.hitPoints > 0){
				...
				if (Input.GetKey (_fpsPlayer.fire) && !waterFireState){
					if(semiAuto){
						if(!semiState){
							Fire();
							semiState = true;
