//SmoothMouseLook.cs by Azuline StudiosВ© All Rights Reserved
...
public class SmoothMouseLook : MonoBehaviour {
    ...
    void Update(){
		...
		if(Time.timeScale > 0 && Time.smoothDeltaTime > 0){//allow pausing by setting timescale to 0
			...
			rotationX += ControlFreak2.CF2Input.GetAxisRaw("Mouse X") * sensitivityAmt * Time.timeScale;//lower sensitivity at slower time settings
			rotationY += ControlFreak2.CF2Input.GetAxisRaw("Mouse Y") * sensitivityAmt * Time.timeScale;
			
			//reset vertical recoilY value if it would exceed maximumY amount 
			if(maximumY - ControlFreak2.CF2Input.GetAxisRaw("Mouse Y") * sensitivityAmt * Time.timeScale < recoilY){
				rotationY += recoilY;
				recoilY = 0.0f;	
			}
			//reset horizontal recoilX value if it would exceed maximumX amount 
			if(maximumX - ControlFreak2.CF2Input.GetAxisRaw("Mouse X") * sensitivityAmt * Time.timeScale < recoilX){
				rotationX += recoilX;
				recoilX = 0.0f;	
			}
