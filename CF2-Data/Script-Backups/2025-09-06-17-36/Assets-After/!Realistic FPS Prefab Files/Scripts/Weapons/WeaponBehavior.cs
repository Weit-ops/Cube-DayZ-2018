//WeaponBehavior.cs by Azuline StudiosВ© All Rights Reserved
...
public class WeaponBehavior : MonoBehaviour {
	...
	public ObscuredInt bulletsLeft = 0;//bullets left in magazine
	[HideInInspector]
	...
	private Animation _mainCameraAnim;
	...
	public void Initialize() {
		...
		_mainCameraAnim = Camera.main.GetComponent<Animation>();
		_aSources = weaponObj.GetComponents<AudioSource>();
		_weaponEffects = weaponObj.GetComponent<WeaponEffects>();
		...
		_playerCapsule = playerObj.GetComponent<CapsuleCollider>();
		_smoothMouseLook = mainCamTransform.parent.transform.GetComponent<SmoothMouseLook>();
		//Access GUIText instance that was created by the PlayerWeapons script
		_ammoText = _playerWeapons.ammoGuiObjInstance.GetComponent<AmmoText>();
		_ammoText2 = _playerWeapons.ammoGuiObjInstance.GetComponentsInChildren<AmmoText>();
		...
		if(!unarmed){
			...
			if(bulletsToReload != bulletsPerClip && weaponMesh.GetComponent<Animation>()["Neutral"]){
	...
	{
		if (!_ironsights.reloading && (int)ammo > 0 && (int)bulletsLeft < (int)bulletsPerClip && shootStartTime + fireRate < Time.time && !ControlFreak2.CF2Input.GetKey(_fpsPlayer.fire) /*&& GameControls.I.Player != null && !GameControls.I.Player.inTheCar*/)
		{
	...
	void OnEnable () {
		//do not perform weapon actions if this is an unarmed/null weapon
		if (unarmed || Time.timeSinceLevelLoad <= 2 || !_playerWeapons.switching)
			return;
		
		StopCoroutine("Reload");//stop reload coroutine if interrupting a non-magazine reload
		_ironsights.reloading = false;//update reloading var in Ironsights script if cancelling reload to fire
					
		//play weapon readying sound
		_aSources[1].volume = 1.0f;
		_aSources[1].pitch = 1.0f * Time.timeScale;
		_aSources[1].clip = readySnd;
		_aSources[1].PlayOneShot(_aSources[1].clip, 1.0f / _aSources[1].volume);
	...
	void FixedUpdate (){
		...
		if(_isInitialized && Time.timeScale > 0 && Time.deltaTime > 0){//allow pausing by setting timescale to 0
			...
			if(meleeSwingDelay == 0 && !unarmed){
				...
				_ammoText.ammoGui = bulletsLeft; //main color
				_ammoText.ammoGui2 = ammo;
				//shadow background color
				_ammoText2[1].ammoGui = bulletsLeft;
				_ammoText2[1].ammoGui2 = ammo;
				//normal position on screen
				_ammoText.horizontalOffsetAmt = _ammoText.horizontalOffset;
				_ammoText.verticalOffsetAmt = _ammoText.verticalOffset;
				_ammoText2[1].horizontalOffsetAmt = _ammoText2[1].horizontalOffset;
				_ammoText2[1].verticalOffsetAmt = _ammoText2[1].verticalOffset;
			}else{
				//make ammo GUIText move off screen if using a melee weapon
				_ammoText.horizontalOffsetAmt = 5f;
				_ammoText.verticalOffsetAmt = 5f;
				_ammoText2[1].horizontalOffsetAmt = 5f;
				_ammoText2[1].verticalOffsetAmt = 5f;
			}
			
			//do not perform weapon actions if this is an unarmed/null weapon
			if(!unarmed){
				
				if(_fpsPlayer.hitPoints >= 1.0f){
					...
					&& !ControlFreak2.CF2Input.GetKeyDown (_fpsPlayer.fire)
					...
					|| _fpsWalker.hideWeapon){//cancel reload if player is climbing, swimming, or holding object and weapon is lowered
				    	if(_ironsights.reloading){
							_ironsights.reloading = false;
					...
					if (ControlFreak2.CF2Input.GetKey (_fpsPlayer.reload) 
					&& !_ironsights.reloading 
					&& ammo > 0 
					&& bulletsLeft < bulletsPerClip
					&& shootStartTime + fireRate < Time.time
					&& !ControlFreak2.CF2Input.GetKey (_fpsPlayer.fire)){
						StartCoroutine("Reload");
					}
					
					if(!_fpsWalker.hideWeapon){//dont shoot if player is climbing
						_ironsights.climbMove = 0.0f;
						//enable/disable shooting based on various player states
						if(!_fpsWalker.sprintActive
						|| _fpsWalker.crouched
						|| (_fpsPlayer.zoomed && meleeSwingDelay == 0)
						|| ((Mathf.Abs(horizontal) > 0) && (Mathf.Abs(vertical) < 1))

						|| (!_fpsWalker.grounded && _fpsWalker.jumping)//don't play sprinting anim while jumping
						|| (_fpsWalker.fallingDistance > 0.75f)//don't play sprinting anim while falling  
						|| ControlFreak2.CF2Input.GetKey (_fpsPlayer.fire)){
							//not sprinting
							...
							//sprinting
							if((!_fpsPlayer.useAxisInput && (ControlFreak2.CF2Input.GetKey (_fpsPlayer.moveForward) || ControlFreak2.CF2Input.GetKey (_fpsPlayer.moveBack)))
							||(_fpsPlayer.useAxisInput && Mathf.Abs(_fpsWalker.inputY) > 0.1f)){
								sprintState = false;
								if(_ironsights.reloading){
									canShoot = false;
								}else{
									if(_fpsPlayer.zoomed && meleeSwingDelay == 0){
										canShoot = true;
									}else{
										canShoot = false;
									}
						...
						if(!_fpsWalker.lowerGunForClimb){
							if(!sprintState){
								recoveryTime = Time.time;
								sprintState = true;
							}
							...
							if(meleeSwingDelay == 0){
								_ironsights.climbMove = -0.4f;
							}else{
								_ironsights.climbMove = -1.4f;
							}
							canShoot = false;
							sprintState = false;
						}
					...
					if (ControlFreak2.CF2Input.GetKey(_fpsPlayer.fire)){
						if((cantFireState)
						&& (canShoot)
						&& (bulletsLeft <= 0)
						&& (ammo <= 0)
						&& ((!PistolSprintAnim && GetComponent<Animation>()["RifleSprinting"].normalizedTime < 0.35f)//only play sound when weapon is centered
						 ||(PistolSprintAnim && GetComponent<Animation>()["PistolSprinting"].normalizedTime < 0.35f))){
							_aSources[1].volume = 1.0f;
							_aSources[1].pitch = 1.0f;
							_aSources[1].clip = noammoSnd;
							_aSources[1].PlayOneShot(_aSources[1].clip, 1.0f / _aSources[1].volume);
					...
					//Change fire mode
					if (ControlFreak2.CF2Input.GetKey(_fpsPlayer.fireMode)){
						if(fireModeState
						&& canShoot
						&& !_ironsights.reloading
					...
					if (barrelSmokePos){
						if(shootStartTime + 0.25f > Time.time){					
							if(barrelSmokeParticles && bulletsJustFired >= barrelSmokeShots ){
								...
								|| ((shootStartTime + fireRate < Time.time) && ControlFreak2.CF2Input.GetKey(_fpsPlayer.fire))){
				...
				if ((canShoot || _fpsWalker.hideWeapon)//allow gun to stay centered and lowered when climbing, swimming, or holding object
					|| _fpsWalker.crouched                                                   
					|| _fpsWalker.midPos < _fpsWalker.standingCamHeight//player is crouching
				    || _ironsights.reloading){
					if(sprintAnimState){//animate weapon up
						//store time that sprint anim started to disable weapon switching during transition
						_playerWeapons.sprintSwitchTime = Time.time;
					...
					if(!sprintAnimState){//animate weapon down
						...
						if(_playerWeapons.switching && _ironsights.reloading){
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void Update (){
		if(_isInitialized && _playerWeapons.CanShoot && Time.timeScale > 0){//allow pausing by setting timescale to 0
			//sync weapon sound effect pitch with timescale/bullet time
			_aSources[1].pitch = 1.0f * Time.timeScale;
			_aSources[0].pitch = 1.0f * Time.timeScale;
			
			//do not perform weapon actions if this is an unarmed/null weapon
			if(!unarmed && _fpsPlayer.hitPoints > 0){
				...
				if (ControlFreak2.CF2Input.GetKey (_fpsPlayer.fire) && !waterFireState){
					if(semiAuto){
						if(!semiState){
							Fire();
							semiState = true;
	...
	void MuzzFlash (){
		...
		if (muzzleFlash){
			...
			if(!_fpsWalker.holdingBreath){
				...
				muzzleFlashColor.r = _playerWeapons.waterMuzzleFlashColor.r;
				muzzleFlashColor.g = _playerWeapons.waterMuzzleFlashColor.g;
				muzzleFlashColor.b = _playerWeapons.waterMuzzleFlashColor.b;
			}
	...
	void Fire (){
		...
		if (!_playerWeapons.CanShoot || bulletsLeft <= 0 || (semiAuto && semiState) || lastReload){
			return;
		}
		...
		if(_fpsWalker.holdingBreath && (!fireableUnderwater || _fpsWalker.lowerGunForSwim)){ 
			if(cantFireState){
				_aSources[1].volume = 1.0f;
				_aSources[1].pitch = 1.0f;
				_aSources[1].clip = noammoSnd;
		...
		if((bulletsToReload == bulletsPerClip && !_ironsights.reloading)
		//allow normal firing when weapon does not reload by magazine
		|| (!_ironsights.reloading && bulletsToReload != bulletsPerClip && bulletsLeft > 0)
		//allow fire button to interrupt a non-magazine reload if there are at least 2 shells loaded
		|| (_ironsights.reloading && bulletsToReload != bulletsPerClip && bulletsLeft >= bulletsToReload * 2.0f && reloadEndTime + reloadTime < Time.time)){
			if (canShoot && !_playerWeapons.switching){//don't allow shooting when reloading, sprinting, or switching
				...
				if((recoveryTime + recoveryTimeAmt < Time.time) && (startTime + readyTime < Time.time)){
					...
					StopCoroutine("Reload");//stop reload coroutine if interrupting a non-magazine reload
					_ironsights.reloading = false;//update reloading var in Ironsights script if cancelling reload to fire
					_aSources[1].clip = null;//stop playing reload sound effect if cancelling reload to fire
	...
	IEnumerator FireOneShot (){
		//do not allow shooting when sprinting
	   	if (canShoot){
			...
			_aSources[0].clip = fireSnd;//play fire sound
			_aSources[0].pitch = Random.Range(0.96f * Time.timeScale, 1.0f * Time.timeScale);//add slight random value to firing sound pitch for variety
			_aSources[0].PlayOneShot(_aSources[0].clip, 0.9f / _aSources[0].volume);//play fire sound

			if (range > 5f)
			{
				PlayerNoiseController.I.AddNoise(NoiseType.Shot);
			}
			...
			for(float i = 0; i < projectileCount; i++){
				...
				if(meleeSwingDelay == 0){
					...
					if(Physics.SphereCast(mainCamTransform.position, _playerCapsule.radius / 3, direction, out hit, range + (range * _fpsWalker.playerHeightMod/2.0f), bulletMask)){
						HitObject(hit, direction);
				...
				if (!hitSomething && WorldController.I.Player != null)
				{
					WorldController.I.Player.ShowPlayerAttack(null);
				}
			}
	...
	void HitObject ( RaycastHit hit, Vector3 direction ){
		...
		case 13:
			{
				...
				if ((bool)component)
				{
					component.Hit((short)damage, direction, mainCamTransform.position, true, WorldController.I.MyId);
					PlayerNoiseController.I.AddNoise(NoiseType.HitZombie);
				}
				break;
			}
		case 25:
			{
				...
				if ((bool)component2)
				{
					component2.HitPlayer((short)damage, (byte)0, WorldController.I.MyId);
				}
				break;
			}
		case 23:
			hit.transform.parent.gameObject.SendMessage("DamageByPlayer", damage, SendMessageOptions.DontRequireReceiver);
			hit.transform.parent.gameObject.SendMessage("Scare", _fpsPlayer.mainCamTransform, SendMessageOptions.DontRequireReceiver);
			break;
		case 21:
		case 28:
			{
				System.Collections.Generic.List<DestructibleObjectType> type = new System.Collections.Generic.List<DestructibleObjectType>();
				if (InventoryController.Instance.Equipment.ContainsKey("Hand"))
				{
					Weapon weaponInfo = WorldController.I.Info.GetWeaponInfo(InventoryController.Instance.Equipment["Hand"]);
					if (weaponInfo != null)
					{
		...
		if(meleeSwingDelay == 0){
			if(!_fpsWalker.holdingBreath){
				_weaponEffects.BulletTracers(direction, muzzleFlash, false);
			}else{
				_weaponEffects.BulletTracers(direction, muzzleFlash, true);
			}
		}
		...
		if(hit.collider.gameObject.tag == "Flesh"){
			...
			if(hitCount < 2){//only draw one flesh impact effect if this is a shotgun for optimization
				_weaponEffects.ImpactEffects(hit);//draw flesh impact effects where the weapon hit NPC
			}
		}else{
			_weaponEffects.ImpactEffects(hit);//draw impact effects where the weapon hit
			_weaponEffects.BulletMarks(hit);//draw a bullet mark where the weapon hit
		}
	...
	IEnumerator Reload (){
		...
		if(Time.timeSinceLevelLoad > 2){//prevent any reloading behavior at level start 
			...
			if((!(_fpsWalker.sprintActive && (Mathf.Abs(horizontal) > 0.75f || Mathf.Abs(vertical) > 0.75f))//allow reload while walking
		     //allow auto reload when sprint button is held, even if stationary
			|| !_fpsWalker.sprintActive)
			&& !_fpsWalker.hideWeapon){//dont allow reloading if gun is lowered while climbing, swimming, or holding object
				...
				if(ammo > 0){//if player has no ammo in their inventory for this weapon, do not proceed with reload
					...
					if(bulletsToReload == bulletsPerClip){
						...
						_aSources[1].volume = 1.0f;
						_aSources[1].pitch = 1.0f * Time.timeScale;
						_aSources[1].clip = reloadSnd;
						_aSources[1].Play();//OneShot(otherfx.clip, 1.0f / otherfx.volume);//play magazine reload sound effect
					...
					if(bulletsToReload == bulletsPerClip){
						...
						if(bulletsNeeded > 0){//if bullets still need to be reloaded and we are not loading a magazine
							...
							_ironsights.reloading = false;//if magazine is full, set reloading var in ironsights script to false
							bulletsReloaded = 0;
							yield return null;//also stop coroutine here to prevent sound from playing below
						}
							
						if(bulletsNeeded <= bulletsToReload || ammo <= 0){//if reloading last round, play normal reloading sound and also chambering effect
							_aSources[1].clip = reloadLastSnd;//set otherfx audio clip to reloadLastSnd
							...
							_aSources[1].clip = reloadSnd;//set otherfx audio clip to reloadSnd
						...
						_aSources[1].volume = 1.0f;
						_aSources[1].pitch = Random.Range(0.95f * Time.timeScale, 1 * Time.timeScale);
						_aSources[1].PlayOneShot(_aSources[1].clip, 1.0f / _aSources[1].volume);
	...
	void WeaponKick (){
		...
		if(_fpsPlayer.zoomed && meleeSwingDelay == 0.0f){
			...
			||(_fpsWalker.crouched && Mathf.Abs(horizontal) == 0.0f && Mathf.Abs(vertical) == 0.0f)){
		...
		if(useRecoilIncrease){
			...
			if(useViewClimb){//apply non-recoverable view climb to mouse input with sustained fire recoil
				if(viewClimbUp > 0.0f){
					_smoothMouseLook.recoilY += ((randYkick / 8.0f * viewClimbUp) * (spreadPerShot / 6.0f ));
				}
				if(viewClimbSide > 0.0f || viewClimbRight > 0.0f){
					_smoothMouseLook.recoilX += ((randXkick / 4.0f * viewClimbSide) + viewClimbRight) * (spreadPerShot / 2.0f);
				}
			}	
		}else{	
			...
			if(useViewClimb){//apply non-recoverable view climb to mouse input without sustained fire recoil
				if(viewClimbUp > 0.0f){
					_smoothMouseLook.recoilY += randYkick / 8.0f * viewClimbUp;
				}
				if(viewClimbSide > 0.0f || viewClimbRight > 0.0f){
					_smoothMouseLook.recoilX += (randXkick / 4.0f * viewClimbSide) + viewClimbRight;
				}
			}	
		}
	}
