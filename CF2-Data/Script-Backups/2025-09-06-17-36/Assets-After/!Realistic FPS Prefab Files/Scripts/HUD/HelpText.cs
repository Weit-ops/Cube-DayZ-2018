//HelpText.cs by Azuline StudiosВ© All Rights Reserved
using UnityEngine;
...
public class HelpText : MonoBehaviour {
	...
	void Update (){
		...
		if(ControlFreak2.CF2Input.GetKeyDown(FPSPlayerComponent.showHelp) && (moveState || moveTime > Time.time)){
			moveState = false;	
			F1pressed = true;
			moveTime = Time.time;
		}
		...
		if(!fadeState && !F1pressed){
			...
			if(ControlFreak2.CF2Input.GetKeyDown(FPSPlayerComponent.showHelp)){
				if(helpTextState){
					if(!helpTextEnabled){
						this.GetComponent<GUIText>().enabled = true;
