...
public class HandUseForRealistic : MonoBehaviour
{
	...
	[SerializeField] LayerMask _useableObjLayer;
	[SerializeField] float _useableRayDistance;
	[SerializeField] List<TextureForUseAction> _cursorTextures;
	[SerializeField] GameObject _hand;
	[SerializeField] Renderer _handRenderer;
	[SerializeField] Transform _itemHolder;
	[SerializeField] PlaceItemViewController _placeItemView;
	[SerializeField] GameObject _myInfoCursor;
	...
	{
		I = this;
		_handRenderer.sharedMaterial.color = CustomizationRegistry.Instance.GetColor(DataKeeper.SkinColorIndex);
	}
	...
	private IEnumerator SaveAfterBuild()
	{
		...
		{
			WorldController.I.SaveWorldInSingle();
		}
	...
	private void HotKeyProcess()
	{
		for (int i = 0; i < 5; i++)
		{
			...
			if (!ControlFreak2.CF2Input.GetKeyDown(keyByNumber))
			{
				continue;
			}
			if (ChatGui.I != null)
			{
				if (!ChatGui.I.chatIsFocused)
				{
	...
	{
		return ControlFreak2.CF2Input.GetKeyDown(GameControls.I.Player.use) && GameControls.I.Player.IsHoverTargetUsableNow;
	}

	public static bool MyUseButton()
	{
		return ControlFreak2.CF2Input.GetKey(GameControls.I.Player.use) && GameControls.I.Player.IsHoverTargetUsableNow;
	}
	...
	private void Update()
	{
		...
		if (ControlFreak2.CF2Input.GetButtonDown("Fire1")){
			if (_info != null && GameControls.I.Walker.CanUseInput){
				Use();
			}
		}
