...
public class ToonWaterDuck:MonoBehaviour{
    public void FixedUpdate() {
     ...
     #endif
    if (ControlFreak2.CF2Input.GetKey ("up")||ControlFreak2.CF2Input.GetKey ("w")) {
    GetComponent<Rigidbody>().AddForce (transform.forward *Time.deltaTime* 1000);
    }
    if (ControlFreak2.CF2Input.GetKey ("down")||ControlFreak2.CF2Input.GetKey ("s")) {
    GetComponent<Rigidbody>().AddForce (transform.forward *Time.deltaTime* -1000);
    }
    if (ControlFreak2.CF2Input.GetKey ("right")||ControlFreak2.CF2Input.GetKey ("d")) {
    GetComponent<Rigidbody>().AddTorque (transform.up *Time.deltaTime* 550);
    }
    if (ControlFreak2.CF2Input.GetKey ("left")||ControlFreak2.CF2Input.GetKey ("a")) {
    GetComponent<Rigidbody>().AddTorque (transform.up *Time.deltaTime* -550);
    }
