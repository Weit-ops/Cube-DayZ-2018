...
#pragma warning restore 649
    ...
    void Update()
    {
        ...
        if (inputEnabled)
        {
            ...
            {
                float mouseWheel = ControlFreak2.CF2Input.GetAxis(MOUSE_WHEEL_AXES_NAME);
                if (mouseWheel != 0)
                {
    ...
    private void CheckInputs()
    {
        ...
        if (inputEnabled)
        {
            int touchCount = ControlFreak2.CF2Input.touchCount;
            if (ControlFreak2.CF2Input.touchCount > 0)
            {
                for (int touchIndex = 0; touchIndex < touchCount; ++touchIndex)
                {
                    ControlFreak2.InputRig.Touch touch = ControlFreak2.CF2Input.GetTouch(touchIndex);
            ...
            else
            {
                ...
                {
                    primaryTouch = new tk2dUITouch(TouchPhase.Began, tk2dUITouch.MOUSE_POINTER_FINGER_ID, ControlFreak2.CF2Input.mousePosition, Vector2.zero, 0);
                    isPrimaryTouchFound = true;
                    isAnyPressBeganRecorded = true;
                }
                else if (ControlFreak2.CF2Input.GetMouseButton(0) || ControlFreak2.CF2Input.GetMouseButtonUp(0))
                {
                    Vector2 deltaPosition = Vector2.zero;
                    TouchPhase mousePhase = TouchPhase.Moved;
                    if (pressedUIItem != null)
                    {
                        deltaPosition = firstPressedUIItemTouch.position - new Vector2(ControlFreak2.CF2Input.mousePosition.x, ControlFreak2.CF2Input.mousePosition.y);
                    }
                    ...
                    secondaryTouch = new tk2dUITouch(mousePhase, tk2dUITouch.MOUSE_POINTER_FINGER_ID, ControlFreak2.CF2Input.mousePosition, deltaPosition, tk2dUITime.deltaTime);
                    isSecondaryTouchFound = true;
                }
            }
        }
        ...
        if (checkForHovers)
        {
            ...
            {
                if (!isPrimaryTouchFound && !isSecondaryTouchFound && hitUIItem == null && !ControlFreak2.CF2Input.GetMouseButton(0)) //if raycast for a button has not yet been done
                {
                    hitUIItem = RaycastForUIItem( ControlFreak2.CF2Input.mousePosition );
                }
                else if (ControlFreak2.CF2Input.GetMouseButton(0)) //if mouse button is down clear it
                {
                    hitUIItem = null;
                }
            }
    ...
    private void CheckMultiTouchInputs()
    {
        ...
        if (inputEnabled)
        {
            ...
            else
            {
                if (ControlFreak2.CF2Input.GetMouseButtonDown(0))
                {
                    allTouches[touchCounter] = new tk2dUITouch(TouchPhase.Began, tk2dUITouch.MOUSE_POINTER_FINGER_ID, ControlFreak2.CF2Input.mousePosition, Vector2.zero, 0);
                    mouseDownFirstPos = ControlFreak2.CF2Input.mousePosition;
                    touchCounter++;
                }
                else if (ControlFreak2.CF2Input.GetMouseButton(0) || ControlFreak2.CF2Input.GetMouseButtonUp(0))
                {
                    Vector2 deltaPosition = mouseDownFirstPos - new Vector2(ControlFreak2.CF2Input.mousePosition.x, ControlFreak2.CF2Input.mousePosition.y);
                    TouchPhase mousePhase = TouchPhase.Moved;
                    ...
                    allTouches[touchCounter] = new tk2dUITouch(mousePhase, tk2dUITouch.MOUSE_POINTER_FINGER_ID, ControlFreak2.CF2Input.mousePosition, deltaPosition, tk2dUITime.deltaTime);
                    touchCounter++;
                }
            }
        }
