...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	//sprinting
	[HideInInspector]
	public bool _sprintActive;//true when sprint button is ready
	[HideInInspector]
	...
	private Animation _mainCamAnim;

	public WeaponBehavior WeaponBehaviorComponent { get; set; }
	...
	void Start (){
		_mainCamAnim = Camera.main.GetComponent<Animation>();
	...
	void Update(){
		if (FPSPlayerComponent.inTheCar)
		{
			return;
		}
		...
		if(ControlFreak2.CF2Input.GetKeyDown (FPSPlayerComponent.crouch) && CanUseInput && !swimming && !climbing){
			if(!crouchState){
				if(!crouched){
					crouched = true;
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			if (FPSPlayerComponent.inTheCar && staminaForSprintAmt >= (float)staminaForSprint)
			{
				return;
			}
			...
			if (!grounded || FPSPlayerComponent.inTheCar || (!sprintActive && limitedSprint))
			{
				if (staminaForSprintAmt < (float)staminaForSprint && Time.time - sprintStopTime >= (float)sprintRegenTime)
				{
					...
					staminaForSprintAmt = Mathf.Min(staminaForSprint, staminaForSprintAmt + Time.deltaTime);
				}
				breathFxState = false;
			}
			...
			if(FPSPlayerComponent.hitPoints > 1.0f){
				...
				if(!Physics.CapsuleCast (sweepPos, sweepHeight, capsule.radius * 0.5f, myTransform.forward, out hit2, 0.4f + (playerHeightMod * 0.2f), clipMask.value) || climbing) {
					if(!FPSPlayerComponent.useAxisInput){
						...
						if(ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveForward) && CanUseInput){inputY = 1;}
						...
						inputYSmoothed = ControlFreak2.CF2Input.GetAxis("Vertical");
				...
				if(!FPSPlayerComponent.useAxisInput){//only check keyboard buttons if not using Unity's input axes
					...
					if (ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveBack) && CanUseInput){inputY = -1;}
					if (!ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveBack) && !ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveForward) || !CanUseInput){inputY = 0;}
					if (ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveBack) && ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveForward) || !CanUseInput){inputY = 0;}
					if (ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.strafeLeft) && CanUseInput){inputX = -1;}
					if (ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.strafeRight) && CanUseInput){inputX = 1;}
					if (!ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.strafeLeft) && !ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.strafeRight) || !CanUseInput){inputX = 0;}
					...
					inputXSmoothed = ControlFreak2.CF2Input.GetAxis("Horizontal");
			...
			if (grounded) {
				...
				if (falling){//reset falling state and perform actions if player has landed from a fall
					...
					if((fallStartLevel - myTransform.position.y) > 2.0f){
						...
						if(!jumping){
							...
							if (_mainCamAnim.IsPlaying("CameraLand")){
								//rewind animation if already playing to allow overlapping playback
								_mainCamAnim.Rewind("CameraLand");
							}
							_mainCamAnim.CrossFade("CameraLand", 0.35f,PlayMode.StopAll);
						}
					}
				...
				if(ControlFreak2.CF2Input.GetKeyDown(FPSPlayerComponent.jump) 
					&& crouched && CanUseInput
				...
				//Sprinting
				/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				bool key = ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.sprint);
				if (key && CanUseInput)
				{
					if (Mathf.Abs(inputY) > 0.1f)
					{
						if ((!FPSPlayerComponent.useAxisInput && ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveForward) && (bool)CanUseInput) || (FPSPlayerComponent.useAxisInput && Mathf.Abs(inputY) > 0.1f))
						{
							FPSPlayerComponent.zoomed = false;
							sprintActive = true;
							if (staminaForSprintAmt > 0f && (bool)limitedSprint)
							{
				...
				else if ((bool)sprintActive)
				{
					sprintActive = false;
				}
				//cancel a sprint in certain situations
				if((sprintActive && ControlFreak2.CF2Input.GetAxis("Fire1") > 0.0f)//cancel sprint if fire button is pressed
					|| (sprintActive && ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.reload) && CanUseInput)//cancel sprint if reload button is pressed
					|| (sprintActive && ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.zoom) && CanUseInput && WeaponBehaviorComponent.canZoom)//cancel sprint if zoom button is pressed
					|| (FPSPlayerComponent.zoomed && ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.fire) && CanUseInput)
				...
				if(sprintActive){
					...
					PlayerNoiseController.I.AddNoise(NoiseType.Run);
				}else{
					if(!swimming){
				...
				if(jumping){
					...
					if(jumpTimer + 0.05f < Time.time){
						...
						if (_mainCamAnim.GetComponent<Animation>().IsPlaying("CameraLand")){
							//rewind animation if already playing to allow overlapping playback
							_mainCamAnim.GetComponent<Animation>().Rewind("CameraLand");
						}

						_mainCamAnim.GetComponent<Animation>().CrossFade("CameraLand", 0.35f,PlayMode.StopAll);
						jumpfxstate = true;
					}
				...
				if (ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.jump) 
				...
				if (!ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.jump) && landStartTime + antiBunnyHopFactor < Time.time){
					jumpBtn = true;
				}
			...
			if((climbing || swimming) && !jumping){//climbing var is managed by a ladder script attatched to a trigger that is placed near a ladder
				if((!FPSPlayerComponent.useAxisInput && ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.moveForward) && CanUseInput) || (FPSPlayerComponent.useAxisInput && inputY > 0)){//only climb if player is moving forward
			...
			if((grounded || climbing || swimming || ((airTime + 0.3f) > Time.time)) && FPSPlayerComponent.hitPoints > 1.0f && !FPSPlayerComponent.restarting){
				...
				if(!capsuleTooSteep || climbing || swimming || (capsuleTooSteep && !rayTooSteep)){
					...
					if(climbing){
						if((!FPSPlayerComponent.useAxisInput && ControlFreak2.CF2Input.GetKey (FPSPlayerComponent.moveForward)) || (FPSPlayerComponent.useAxisInput && Mathf.Abs(inputY) > 0.1f)){//move player up climbable surface if pressing forward button
							velocityChange.y = verticalSpeedAmt;
						}else if(ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.jump) && CanUseInput){//move player up climbable surface if pressing jump button
							inputY = 1;//to cycle bobbing effects
							if(FPSPlayerComponent.useAxisInput){inputYSmoothed = 1.0f;}
							velocityChange.y = climbSpeed * 0.75f;
						}else if(ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.crouch) && CanUseInput){//move player down climbable surface if pressing crouch button
							inputY = -1;//to cycle bobbing effects
							if(FPSPlayerComponent.useAxisInput){inputYSmoothed = 1.0f;}
							velocityChange.y = -climbSpeed * 0.75f;
						}else{
			...
			if(!climbing){
				if(!swimming){
					...
					if(swimStartTime + 0.2f > Time.time){//make player sink under surface for a short time if they jumped in deep water 
						...
						if(ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.jump) && CanUseInput){
						...
						}else if(ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.crouch) && CanUseInput){
						...
						if(!belowWater && !ControlFreak2.CF2Input.GetKey(FPSPlayerComponent.jump)){
							canWaterJump = true;	
						}
