...
public class FPSPlayer : MonoBehaviour {
	...
	void Update (){
		...
		if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.E) && inTheCar && canUseExit /*&& myCurentCarWrapper != null && myCurrentDoor != null*/)
		{
		...
		{
			if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.Equals) || ControlFreak2.CF2Input.GetKeyUp(KeyCode.KeypadPlus))
			{
				JsSpeeker.I.PlayNextSong();
			}
			if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.Minus) || ControlFreak2.CF2Input.GetKeyUp(KeyCode.KeypadMinus))
			{
				JsSpeeker.I.PlayPrevSong();
			}
			if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.Backspace))
			{
	...
	void FixedUpdate (){
		...
		if (ControlFreak2.CF2Input.GetKey (exitGame) && CanUseInput){
			Application.Quit();
		}
		
		//Restart level if v is pressed
		if (ControlFreak2.CF2Input.GetKey (restartScene) && CanUseInput){
		...
		if(ControlFreak2.CF2Input.GetKey (zoom) && CanUseInput
		...
		if (ControlFreak2.CF2Input.GetKey(use)){
			pickUpBtnState = false;
		}else{
			pickUpBtnState = true;	
		}
